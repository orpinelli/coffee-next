...
export function ProductGrid({ category, searchParams }: ProductGridProps) {
	...
	if (searchParams.marca) {
		filteredProducts = filteredProducts.filter(
			(product) =>
				product.brand.toLowerCase() === searchParams.marca?.toLowerCase(),
		);
	}

	if (searchParams.preco_min) {
		filteredProducts = filteredProducts.filter(
			(product) => product.price >= Number.parseFloat(searchParams.preco_min!),
		);
	}

	if (searchParams.preco_max) {
		filteredProducts = filteredProducts.filter(
			(product) => product.price <= Number.parseFloat(searchParams.preco_max!),
		);
	}
